...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self,
        ...
        if torch.isnan(cost).any() or torch.isinf(cost).any():
            ...
            print('Warning:nan or inf in assignment cost!')
            cost = torch.nan_to_num(cost, nan=1e6)
            cost = torch.clip(cost,-1e6,1e6)

            if torch.min(cost) < 0.01:
                print('Warning: cost too small!')
                cost += torch.abs(torch.min(cost))+0.01
