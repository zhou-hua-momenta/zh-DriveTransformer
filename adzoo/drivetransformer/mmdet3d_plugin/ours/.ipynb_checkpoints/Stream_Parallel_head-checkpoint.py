...
class StreamParallelHead(BaseModule):
    def __init__(
        self,
        # Hyper-Parameter
        ## Model Size
        ...
        ## Motion
        self.agent_traj_fus = nn.Sequential(
            nn.Linear(self.fut_ts, self.embed_dims),
            nn.SiLU(inplace=True),
            nn.Linear(self.embed_dims, self.embed_dims),
            nn.SiLU(inplace=True),
            nn.Linear(self.embed_dims, self.embed_dims),
        ...
        self.map_cls_embedding = nn.Sequential(
            ...
            nn.SiLU(inplace=True),
            nn.Linear(self.embed_dims, self.embed_dims),
            #LayerScale(self.embed_dims, init_values=1e-2)
        ) # Map Cls PE
    ...
    def forward(self,
        ...
        if ego_lcf_feat.dim() == 4:
            speeds = ego_lcf_feat[:,0,0,-2].detach().clone()
        else:
            speeds = ego_lcf_feat[0,0,-2].unsqueeze(0).detach().clone()
        ...
        ## ego_his_traj: (1,1,2,2)
        ## ego_his_cmd: (1,1,1,3)
        ego_query = self.ego_lcf_encoder(torch.cat([ego_lcf_feat.squeeze(1)[..., self.ego_lcf_feat_idx], ego_his_trajs.flatten(-2, -1), ego_fut_cmd.squeeze(1)], dim=-1))
        ...
        if self.ego_multi_modal:
            ...
            ego_query = (ego_query.unsqueeze(2) + ego_mode_query.unsqueeze(1)).flatten(1,2) 
   
        #ego_temp_pos = ego_temp_pos.unsqueeze(2).expand(-1, -1, self.fut_mode, -1).flatten(1,2)
        ...
        agent_query, map_query, ego_query, results = self.transformer(
            ...
            ego_ref,
            agent_prep_traj_ref,
            ego_prep_traj_ref_fix_time,
            ego_prep_traj_ref_fix_dist,
            mode_query=mode_query,
            agent_cls=agent_prep_class,
            map_cls=map_prep_class,
            agent_traj_fus=self.agent_traj_fus,
            #agent_mode_fus=self.agent_mode_fus,
            #ego_mode_fus=self.ego_mode_fus,
            #map_query_fus=self.map_query_fus,
            #map_pts_fus=self.map_pts_fus,
            agent_ref_embedding=self.agent_ref_embedding,
        ...
        outs = {
            'all_cls_scores': agent_class,
            'all_bbox_preds': agent_coords,
            'all_traj_preds': agent_traj_coords.view(agent_traj_coords.size(0), agent_traj_coords.size(1), 
                                                     self.agent_num_query_sifted, self.fut_mode, self.fut_ts, 2).flatten(-2,-1),
            'all_traj_cls_scores': agent_traj_cls.unsqueeze(-1).view(agent_traj_cls.size(0), agent_traj_cls.size(1), self.agent_num_query_sifted, self.fut_mode),
            'map_all_cls_scores': map_class,
            'map_all_bbox_preds': map_coords,
            'map_all_pts_preds': map_pts_coords,
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            'map_enc_cls_scores': None,
            'map_enc_bbox_preds': None,
            'map_enc_pts_preds': None,
            'ego_fut_preds_fix_time': ego_traj_fix_time,
            'ego_fut_preds_fix_dist': ego_traj_fix_dist,
            'ego_traj_cls_scores': ego_traj_cls,
            'map_pre_cls_scores': map_prep_class,
            'map_pre_coord_preds': map_prep_box,
            'map_pre_pts_coord_preds': map_prep_pts_coord,
            'agent_pre_cls_scores': agent_prep_class,
            'agent_pre_coord_preds': agent_prep_ref,
        }
    ...
    def img_3d_position_embedding(self, data, memory_centers, img_metas, topk_indexes=None):
        ...
        #pad_h, pad_w, _ = data['pad_shape'][0, 0]
    ...
    def prepare_location(self, img_feats, data):
        if self.img_patch_location is None:
            #pad_h, pad_w, _ = data['pad_shape'][0, 0] (384, 640, 3)
            bs, n = img_feats.shape[:2]
            pad_h, pad_w = img_feats.shape[-2]*self.img_stride, img_feats.shape[-1]*self.img_stride
            self.pad_h = pad_h
            self.pad_w = pad_w
            self.img_patch_location = locations(img_feats.size()[-2:], img_feats.device, self.img_stride, pad_h, pad_w).detach()#[None].repeat(bs*n, 1, 1, 1)
    ...
    def prepare_anchor_ref(self):
        ...
        z_coords = 0 * torch.ones_like(x_coords, device=self.pc_range.device)#0.5 * self.pc_range[5] * torch.ones_like(x_coords, device=self.pc_range.device) ## TODO_0513_DONE: 0 * torch.ones_like(x_coords, device=self.pc_range.device)
        ego_ref = []
        ...
        # pc_range->[0,1]
        #ego_ref = (ego_ref - self.pc_range[:3]) / (self.pc_range[3:6] - self.pc_range[0:3])  ## 
        return ego_ref
    ...
    def pre_update_memory(self, data):
        with torch.no_grad():
            ...
            # memory length differs for different types of queries, but the stored number of
            # history frames is the same
            ...
            if self.agent_memory_embedding is None:
                ...
                #self.ego_memory_reference_point = x.new_zeros(B, self.memory_len_frame, 3)
            ...
            #if self.agent_num_propagated > 0:
                # self.agent_memory_reference_point[:, :self.num_propagated]  = self.agent_memory_reference_point[:, :self.num_propagated] + torch.zeros((B,self.num_propagated,3),device=x.device)   
                # self.map_memory_reference_point[:, :self.num_propagated]  = self.map_memory_reference_point[:, :self.num_propagated] + (1-x).view(B, 1, 1) * pseudo_reference_points
    ...
    def post_update_memory(
        ...
        with torch.no_grad():
            ...
            agent_ref_point = torch.cat(
                [agent_bbox_preds[..., :2], agent_bbox_preds[..., 4:5]], dim=-1
            )[-1]
            ...
            map_ref_point = torch.cat(
                [map_ref_preds[..., :2], torch.zeros_like(map_ref_preds[..., 0:1])], dim=-1
            )[-1]
    ...
    def temporal_alignment(self, agent_query, map_query):
        ...
        # pc_range -> [0, 1]
        # agent_temp_reference_point = (self.agent_memory_reference_point - self.pc_range[:3]) / (self.pc_range[3:6] - self.pc_range[0:3])
        # map_temp_reference_point = (self.map_memory_reference_point[...,0:2] - self.pc_range[:2]) / (self.pc_range[3:5] - self.pc_range[0:2])
        # ego_temp_reference_point = (self.memory_egopose[:,:,0:3,3] - self.pc_range[:3]) / (self.pc_range[3:6] - self.pc_range[0:3])
        ...
        #ego_temp_reference_yaw = torch.arctan2(self.memory_egopose[:,:,1,0],self.memory_egopose[:,:,1,1]).unsqueeze(-1)
        agent_temp_pos = self.agent_ref_embedding(pos2posemb(agent_temp_reference_point, self.embed_dims//2))#self.agent_ref_embedding(pos2posemb(torch.cat([agent_temp_reference_point, self.agent_memory_reference_yaw],dim=-1), self.embed_dims//2))
        map_temp_pos = self.map_ref_embedding(pos2posemb(map_temp_reference_point, self.embed_dims//2))
        ego_temp_pos = self.agent_ref_embedding(pos2posemb(ego_temp_reference_point, self.embed_dims//2))#self.agent_ref_embedding(pos2posemb(torch.cat([ego_temp_reference_point,ego_temp_reference_yaw],dim=-1), self.embed_dims//2))
        ...
        if self.with_ego_pos:
            # rec_ego_motion = torch.cat([torch.zeros_like(agent_query[..., :3]), rec_ego_pose[..., :3, :].flatten(-2)], dim=-1)
            # rec_ego_motion = nerf_positional_encoding(rec_ego_motion)
            #agent_query = self.ego_pose_memory(agent_query, rec_ego_motion)
            memory_ego_motion = []
        ...
        agent_temp_pos += time_emded
        agent_temp_pos += agent_cls_emded
        assert self.agent_num_propagated >= self.map_num_propagated
        map_temp_pos += time_emded[:, :self.map_num_propagated*self.memory_len_frame]
    ...
    def _get_target_single(self,
        ...
        # if num_gt_bbox == 0:
        #     gt_fut_masks = gt_fut_masks.unsqueeze(-1).repeat(1, 1, 2).view(num_gt_bbox, self.fut_ts*2)
        #     sampling_result.pos_gt_bboxes = torch.tensor([], device=gt_fut_masks.device).unsqueeze(-1).repeat(1, 1, 9)
        # else:
    ...
    def map_loss_single(self,
        ...
        #     normalized_bbox_preds[isnotnan, :4],
        ...
        loss_cls = torch.nan_to_num(loss_cls)
        loss_bbox = 0#torch.nan_to_num(loss_bbox)
        loss_iou = 0#torch.nan_to_num(loss_iou)
    ...
    def loss(self,
        ...
        l0_loss_map_cls_all, l0_loss_map_bbox_all, l0_loss_map_iou_all, l0_loss_map_pts_all, l0_loss_map_dir_all = self.map_loss_single(map_pre_cls_scores, map_pre_coord_preds, map_pre_pts_coord_preds, map_all_gt_bboxes_list[0], map_all_gt_labels_list[0],map_all_gt_shifts_pts_list[0], map_all_loss_gt_mask[0], map_all_gt_bboxes_ignore_list[0])
        ...
        #loss_dict['d0.loss_map_bbox_all'] = l0_loss_map_bbox_all
        #loss_dict['d0.loss_map_iou_all'] = l0_loss_map_iou_all
