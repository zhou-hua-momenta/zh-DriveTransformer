...
import torch
import warnings
from mmcv.utils import get_dist_info, init_dist, wrap_fp16_model, set_random_seed, Config, DictAction, load_checkpoint, ProgressBar
...
def main():
    ...
    if args.remove_orig_mod:
        tmp_checkpoint = torch.load(args.checkpoint, map_location="cpu")
        ...
        torch.save(remapped_checkpoint, str(args.checkpoint[:-4])+"_remapped.pth")
        args.checkpoint = str(args.checkpoint[:-4])+"_remapped.pth"
        print("Remap checkpoint to", str(args.checkpoint))
