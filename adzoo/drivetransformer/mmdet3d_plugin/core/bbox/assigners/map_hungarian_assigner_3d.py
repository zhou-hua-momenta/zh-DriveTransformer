...
class MapHungarianAssigner3D(BaseAssigner):
    ...
    def assign(self,
        ...
        normalized_bbox_pred = normalize_2d_bbox(bbox_pred, self.pc_range)
        reg_cost = self.reg_cost(normalized_bbox_pred[:, :4], normalized_gt_bboxes[:, :4])

        _, num_orders, num_pts_per_gtline, num_coords = gt_pts.shape
        normalized_gt_pts = normalize_2d_pts(gt_pts, self.pc_range)
        normalized_pts_pred =  normalize_2d_pts(pts_pred, self.pc_range)
        num_pts_per_predline = normalized_pts_pred.size(1)
        if num_pts_per_predline != num_pts_per_gtline:
            normalized_pts_pred_interpolated = F.interpolate(normalized_pts_pred.permute(0,2,1),size=(num_pts_per_gtline),
                                            mode='linear', align_corners=True)
            normalized_pts_pred_interpolated = normalized_pts_pred_interpolated.permute(0,2,1).contiguous()
        else:
            normalized_pts_pred_interpolated = normalized_pts_pred
        # num_q, num_pts, 2 <-> num_gt, num_pts, 2
        pts_cost_ordered = self.pts_cost(normalized_pts_pred_interpolated, normalized_gt_pts)
