...
def format_res_gt_by_classes(result_path,
    ...
    if len(gen_results) != len(annotations):
        print(f"-----------------Not enough samples ({len(gen_results)}) while the number of GT is ({len(annotations)})")
        new_annotations = []
        for key in gen_results:
            for annotation in annotations:
                if key == annotation["sample_token"]:
    ...
    with open(formatting_file, 'wb') as f:
        pickle.dump([cls_gens, cls_gts], f)
